# retrieve opa repo contributions: adapted from Hadley Wickham's scripts to credit contributors to R for Data Science
...
#JSON format info on contributors to template repo
opa_json <- gh::gh("/repos/:owner/:repo/contributors",
                    owner = "BITSS-OPA",
                    repo = "opa-template",
                    .limit = Inf)
opa_contribs <- tibble(
  login = opa_json %>% map_chr("login"),
  n = opa_json %>% map_int("contributions")
)
